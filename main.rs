...
impl DVHOST {
    pub fn new(host: String, ip: String, username: String, password: String) -> Self {
        ...
        instance.inbounds.insert("vmess".to_string(), vec![
            "VMess TCP".to_string(),
            "VMess Websocket".to_string()
        ]);
        instance.inbounds.insert("vless".to_string(), vec![
            "VLESS TCP REALITY".to_string(),
            "VLESS GRPC REALITY".to_string()
        ]);
        instance.inbounds.insert("shadowsocks".to_string(), vec![
            "Shadowsocks TCP".to_string()
        ]);
    ...
    pub fn add_user(
        ...
        let data = json!({
            "username": username,
            "proxies": self.proxies(vmess, vless, shadow_socks),
            "inbounds": self.inbounds(vmess, vless, shadow_socks),
            "expire": current_time + expire_days,
            "data_limit": volume_bytes,
            "data_limit_reset_strategy": "no_reset",
            "status": if status { "active" } else { "disabled" },
            "note": note,
            "on_hold_timeout": Value::Null,
    ...
    pub fn edit_user(&self, username: &str, update: Value) -> DVHOSTResponse {
        ...
        let toggle_status = update.get("toggle_status")
            .map(|v| v.as_bool().unwrap_or(false))
            .unwrap_or(false);
        
        let final_status = if toggle_status { !status } else { status };
        
        let expire = update.get("expire")
            .map(|v| v.as_i64().unwrap_or(0))
            .unwrap_or(user["expire"].as_i64().unwrap_or(0));
        
        let add_days = update.get("add_days")
            .map(|v| v.as_i64().unwrap_or(0))
            .unwrap_or(0);
        
        let data_limit = update.get("volume")
            .map(|v| v.as_f64().unwrap_or(0.0))
            .unwrap_or(user["data_limit"].as_f64().unwrap_or(0.0));
        
        let add_volume = update.get("add_volume")
            .map(|v| v.as_f64().unwrap_or(0.0))
            .unwrap_or(0.0);
    ...
    fn auth_token(&self, username: &str, password: &str) -> Option<String> {
        ...
        if response.status == 200 {
            response.data["access_token"].as_str().map(|s| s.to_string())
        } else {
            None
        }
    }
